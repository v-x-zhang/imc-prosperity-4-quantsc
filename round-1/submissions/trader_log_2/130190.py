...
class Trader:
    POSITION_LIMITS: Dict[str, int] = {
        "ASH_COATED_OSMIUM": 1000000,
        "INTARIAN_PEPPER_ROOT": 1000000,
    }

    # PEPPER drifts +1000 over 1M timestamps = +1 per 1000 timestamps
    PEPPER_DRIFT_RATE = 1000 / 1_000_000  # per timestamp unit
    ...
    def run(self, state: TradingState):
        ...
        for product in state.order_depths:
            ...
            if product == "ASH_COATED_OSMIUM":
                ...
                # On first tick, anchor theo to current mid; then drift forward
                key = "pepper_anchor"
                if key not in trader_state:
                    trader_state[key] = mid  # anchor to first observed mid
                    trader_state["pepper_t0"] = state.timestamp
                anchor = trader_state[key]
                t0 = trader_state["pepper_t0"]
                theo = anchor + (state.timestamp - t0) * self.PEPPER_DRIFT_RATE
            else:
                theo = mid
