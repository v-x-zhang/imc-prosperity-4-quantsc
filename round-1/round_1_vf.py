...
class Trader:
    ...
    def run(self, state: TradingState) -> tuple[dict[Symbol, list[Order]], int, str]:
        ...
        for product in state.order_depths:
            ...
            if product == "ASH_COATED_OSMIUM":
                best_bid_mid = max(order_depth.buy_orders) if order_depth.buy_orders else None
                best_ask_mid = min(order_depth.sell_orders) if order_depth.sell_orders else None
                if best_bid_mid is not None and best_ask_mid is not None:
                    mid = (best_bid_mid + best_ask_mid) / 2.0
                ...
                anchor = trader_state.get("ash_ewma", self.ASH_ANCHOR_SEED)
    ...
    def _trade_ash(self, order_depth: OrderDepth, pos: int, limit: int, product: str, anchor: float) -> list[Order]:
        orders: list[Order] = []
        buy_room = limit - pos
        sell_room = limit + pos
        ...
        if book_asks:
            ...
            for ask_price in sorted(book_asks.keys()):
                if buy_room <= 0:
                    break
                take = ask_price <= anchor - self.ANCHOR_WIDTH
                flatten = (
        ...
        if book_bids:
            ...
            for bid_price in sorted(book_bids.keys(), reverse=True):
                if sell_room <= 0:
                    break
                take = bid_price >= anchor + self.ANCHOR_WIDTH
                flatten = (
