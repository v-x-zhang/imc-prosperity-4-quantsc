...
# Config
PARAM_1 = "FLATTEN_THRESHOLD"
GRID_1 = [0, 20, 40, 60, 80]

PARAM_2 = "FLATTEN_WIDTH"
GRID_2 = [-1, 0, 1]
...
# Main
def main():
    print(f"2D grid search: {PARAM_1} x {PARAM_2}")
    ...
    # results[(v1, v2)] = (metric, pnl, sharpe, day_pnls)
    results: dict[tuple, tuple] = {}

    total = len(GRID_1) * len(GRID_2)
    i = 0
    for v1 in GRID_1:
        for v2 in GRID_2:
            i += 1
            tmp_path = patch_file(src, {PARAM_1: v1, PARAM_2: v2})
            try:
                r = run_backtest(tmp_path)
                metric_val = r[METRIC]
                results[(v1, v2)] = (metric_val, r["pnl"], r["sharpe"], r["days"])
                metric_str = f"{metric_val:.4f}" if metric_val is not None else "N/A"
                days_str = "[" + ", ".join(f"{d:>8,.0f}" for d in r["days"]) + "]" if r["days"] else "[]"
                print(
                    f"  [{i:>3}/{total}] {PARAM_1}={v1:<6} {PARAM_2}={v2:<3} "
                    f"→  PnL={r['pnl'] if r['pnl'] is not None else 'N/A':>10} "
                    f"Sharpe={r['sharpe'] if r['sharpe'] is not None else 'N/A':>7}  "
                    f"days={days_str}"
                )
            except Exception as e:
                print(f"  [{i:>3}/{total}] {PARAM_1}={v1} {PARAM_2}={v2} → ERROR: {e}")
                results[(v1, v2)] = (None, None, None, [])
            finally:
                os.unlink(tmp_path)

    print("=" * 80)

    # Matrix print (rows = PARAM_1, cols = PARAM_2)
    ...
    _print_matrix(
        f"{METRIC.upper()} matrix (rows={PARAM_1}, cols={PARAM_2}):",
        lambda v1, v2: _fmt(results[(v1, v2)][0]),
    )
    ...
    for d in range(n_days):
        _print_matrix(
            f"Day {d} PnL (rows={PARAM_1}, cols={PARAM_2}):",
            lambda v1, v2, d=d: _fmt(results[(v1, v2)][3][d] if d < len(results[(v1, v2)][3]) else None),
        )

    # Best
    valid = [(k, r) for k, r in results.items() if r[0] is not None]
    if valid:
        (best_v1, best_v2), (best_m, best_p, best_s, best_d) = max(valid, key=lambda kv: kv[1][0])
        print(
            f"\nBEST: {PARAM_1}={best_v1}, {PARAM_2}={best_v2}  "
